...
if [ "$ROLE_NGINX" = "true" ]; then
    ...
    if [ -n "$REDIS_HOST" ]; then 
        ...
        # Use template but remove start from upstream redis-docker into } 
        envsubst  < /etc/nginx/conf.d/upstream.conf.template > /etc/nginx/conf.d/upstream.conf
        sed -i '/upstream redis-docker {/,/}/d' /etc/nginx/conf.d/upstream.conf
    fi
fi
...
if [ "$ROLE_PHP_FPM" = "true" ]; then
    ...
    if [ -n "$REDIS_PASSWORD" ]; then
        sed -i "s/^session.save_path =.*/session.save_path = \"$redis_scheme:\/\/${REDIS_HOST}:6379?auth=${REDIS_PASSWORD}\"/" /etc/php/8.2/fpm/conf.d/custom.ini
    else
        sed -i "s/^session.save_path =.*/session.save_path = \"$redis_scheme:\/\/${REDIS_HOST}:6379\"/" /etc/php/8.2/fpm/conf.d/custom.ini
    fi    
